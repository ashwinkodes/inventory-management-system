...
const EditGearModal = ({ gear, onClose, onSuccess }: EditGearModalProps) => {
  const [formData, setFormData] = useState({
    name: gear.name || '',
    brand: gear.brand || '',
    model: gear.model || '',
    category: gear.category || 'BACKPACK',
    description: gear.description || '',
    condition: gear.condition || 'GOOD',
    size: gear.size || '',
    weight: gear.weight || '',
    clubId: gear.clubId || 'default-club',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(error.response?.data?.error || 'Failed to update gear item');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              {gear.imageUrl ? 'Replace Image' : 'Add Image'}
            </label>
